# Claude Code 任务报告
- 时间: 2026-02-13 19:05:47
- Session: b83a417f-a8a6-4b84-9615-4e727db27fc4
- Stop Reason: unknown
- transcript: /home/claw/.claude/projects/-home-claw--openclaw-workspace/b83a417f-a8a6-4b84-9615-4e727db27fc4.jsonl (7585067 bytes, 2605 lines)
...
**Root cause:** The `project_boms` table had a CHECK constraint `ck_bom_type` that only allowed `EBOM`, `SBOM`, `OBOM`, `FWBOM` — no `PBOM` or `MBOM`. When `ProcessBOMUploadFields` tried to create a BOM with `bom_type='PBOM'`, the insert failed at the DB level. The BOM that appeared was likely created from a prior form submission when the field was configured as SBOM.

**Fix:**
1. **`cmd/plm/main.go`** — Added migration SQL to drop and recreate `ck_bom_type` constraint with all valid types: `EBOM`, `SBOM`, `PBOM`, `MBOM`, `OBOM`, `FWBOM`
2. **Data fix** — Updated the existing mistyped BOM from SBOM to PBOM for the Zenith project

## 文件改动
统计: 30 files, ~2342 lines written, ~1809 lines edited
文件列表:
...
## 工具调用（最近30条）
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -t -c "SELECT tf.task_id, t.name, tf.fields::text FROM 
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -t -c "SELECT id, name, status, project_id FROM tasks W
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -t -c "SELECT id, bom_type, name FROM project_boms WHER
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -t -c "SELECT data::text FROM task_form_submissions WHE
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -t -c "SELECT data::text FROM task_form_submissions WHE
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -t -c "SELECT count(*) FROM project_bom_items WHERE bom
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -t -c "SELECT id, name, bom_type, created_at FROM proje
→ Read: /home/claw/.openclaw/workspace/internal/plm/service/project_service.go
→ Read: /home/claw/.openclaw/workspace/internal/plm/service/project_service.go
→ Read: /home/claw/.openclaw/workspace/internal/plm/service/bom_service.go
→ Read: /home/claw/.openclaw/workspace/internal/plm/service/project_service.go
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -t -c "SELECT id, name, bom_type, created_by, created_a
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -c "UPDATE project_boms SET bom_type='PBOM' WHERE id='e
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -t -c "SELECT conname, pg_get_constraintdef(oid) FROM p
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -c "
ALTER TABLE project_boms DROP CONSTRAINT IF EXISTS
→ Bash: PGPASSWORD=nimo123 psql -h 127.0.0.1 -p 5432 -U nimo -d nimo_plm -c "UPDATE project_boms SET bom_type='PBOM' WHERE id='e
